...
def direct_prediction(claim, evidence):
    """Run the direct fact-verification baseline."""
...
def basic_self_refinement(claim, evidence, initial_answer):
    """Run generic feedback followed by one refinement pass."""
    feedback = call_llm(basic_feedback_prompt(claim, evidence, initial_answer))
    refined_answer = call_llm(
        basic_refine_prompt(claim, evidence, initial_answer, feedback)
    )
    return {
...
def evidence_aware_self_refinement(claim, evidence, initial_answer):
    """Run evidence-aware feedback followed by one refinement pass."""
    feedback = call_llm(
        evidence_aware_feedback_prompt(claim, evidence, initial_answer)
    )
    refined_answer = call_llm(
        evidence_aware_refine_prompt(claim, evidence, initial_answer, feedback)
    )
    return {
        "initial_answer": initial_answer,
        "feedback": feedback,
...
def vitaminc_style_self_refinement(claim, evidence, initial_answer):
    """Run VitaminC-style feedback followed by one refinement pass."""
    feedback = call_llm(
        vitaminc_style_feedback_prompt(claim, evidence, initial_answer)
    )
    refined_answer = call_llm(
        vitaminc_style_refine_prompt(claim, evidence, initial_answer, feedback)
    )
    return {
        "initial_answer": initial_answer,
        "feedback": feedback,
...
def evaluate(results, method_name):
    """Return accuracy for one method in a result list."""
    if not results:
        return 0.0
...
def label_counter(results, method_name):
    """Return the prediction distribution for one method."""
    predictions = []
    for row in results:
        if method_name == "direct":
        ...
        predictions.append(normalize_label(pred))
