#-----------baseline + Word-level Candidate Rationale 전용 실행 파일임 -----------------
# --------------------MODEL------------------------
# direct
# basic_self_refine
# evidence_aware_self_refine
# vitaminc_style_self_refine
# wordcand_self_refine  ← Word-level Candidate Rationale
# -------------------실행 방법------------------------
# - llm_client_wordr.py 사용
# - call_llm_with_logprobs 사용
# - direct 단계에서 initial_conf 저장
# - rationale_refine_word_cand.py의 wordcand_self_refinement 호출
# - VitaminC-style cue 기반 candidate 통계 출력
# -----------------------------------------------------
# cue 유형 예시 (VitaminC style):
# negation: not, never, without
# directionality: increase, decrease, inhibit, promote
# temporal: before, after, previous
# numerical: %, more than, less than
# scope: only, all, some, must, may
# relation: born in, caused by, member of
# --------------------------------------------------

"""
run_experiment_wordr.py
========================
기존 run_experiment.py에 Word-level Rationale Self-Refinement (WordR) 방식 추가.

기존 4가지 방식은 그대로 유지:
    direct / basic_self_refine / evidence_aware_self_refine / vitaminc_style_self_refine

새로 추가:
    wordr_self_refine  ← Word-level Perturbation 기반

주요 변경:
    - llm_client_wordr.py 사용 (call_llm_with_logprobs 지원)
    - direct_prediction_with_conf(): initial_conf도 함께 반환
    - wordr_self_refinement() 호출 및 결과 저장
    - 평가/통계 출력에 wordr 방식 포함

Hyperparameter:
    RATIONALE_TOP_K              = 5     # Top-K rationale 수
    CONFIDENCE_DROP_THRESHOLD    = 0.2   # dev set ablation 권장: 0.1/0.15/0.2/0.25/0.3
"""
...
# ── 새 방식 ──────────────────────────────────────────────────
from rationale_refine_word_cand import wordcand_self_refinement
...
RATIONALE_TOP_K           = 5
CONFIDENCE_DROP_THRESHOLD = 0.3   # ablation: 0.1 / 0.15 / 0.2 / 0.25 / 0.3
...
def parse_decision(output):
    """Parse KEEP / CHANGE decision from refinement output."""
    if output is None:
        return "UNKNOWN"

    match = re.search(
        r"Decision:\s*\[?\s*(KEEP|CHANGE)\s*\]?",
        str(output),
        re.IGNORECASE,
    )
    if match:
...
def basic_self_refinement(claim, evidence, initial_answer, num_iterations=1, early_stop=False):
    ...
    for it in range(1, num_iterations + 1):
        ...
        decision = parse_decision(refined_answer)
        ...
        if early_stop and decision == "KEEP":
            break
    ...
    return {
        ...
        "actual_iterations": len(history),
        "early_stop": early_stop,
        "stopped_early": early_stop and len(history) < num_iterations,
        "stop_reason": "KEEP" if (early_stop and history and history[-1].get("decision") == "KEEP") else "max_iterations",
        "iteration_history": history,
    }


def evidence_aware_self_refinement(claim, evidence, initial_answer, num_iterations=1, early_stop=False):
    current_answer = initial_answer
    history = []
...
def vitaminc_style_self_refinement(claim, evidence, initial_answer, num_iterations=1, early_stop=False):
    current_answer = initial_answer
    history = []
...
def label_counter(results, method_name):
    ...
    for row in results:
        if method_name == "direct":
            preds.append(row["direct"]["label"])
        else:
            preds.append(row[method_name]["refined_label"])
...
def main():
    ...
    parser.add_argument("--num-iterations", type=int, default=1,
                        help="Maximum number of refinement iterations for baseline self-refine methods")
    parser.add_argument("--early-stop", action="store_true",
                        help="Stop baseline self-refine early when the refined answer returns Decision: KEEP")
    ...
    print(f"[Baseline Self-Refine] max_iterations={args.num_iterations}, early_stop={args.early_stop}")
    ...
    for idx, sample in enumerate(samples, start=1):
        ...
        print(
            f"  [Basic]          {row['basic_self_refine']['refined_label']} "
        ...
        print(
            f"  [VitaminC]       {row['vitaminc_style_self_refine']['refined_label']} "
            f"(iter={args.num_iterations})",
            flush=True,
        )

        # ── WordR 방식 ────────────────────────────────────────
        row["wordcand_self_refine"] = wordcand_self_refinement(
            claim                     = claim,
            evidence                  = evidence,
            initial_answer            = initial_answer,
        ...
        row["wordcand_self_refine"]["initial_conf"] = initial_conf
        ...
        print(
            f"  [WordCand]          {row['wordcand_self_refine']['refined_label']} "
            f"| candidates={row['wordcand_self_refine'].get('total_candidates',0)} "
            f"| verified={row['wordcand_self_refine'].get('verified_spans',[])}",
            flush=True,
        )
    ...
    total_cands = sum(
        r["wordcand_self_refine"].get("total_candidates", 0)
        for r in results
    )
    total_verified = sum(
        r["wordcand_self_refine"].get("num_verified", 0)
        for r in results
    )
    zero_candidates = sum(
        1 for r in results
        if r["wordcand_self_refine"].get("total_candidates", 0) == 0
    )
    zero_verified = sum(
        1 for r in results
        if r["wordcand_self_refine"].get("num_verified", 0) == 0
    )
    label_changed = sum(
        1 for r in results
        if r["wordcand_self_refine"].get("label_changed_after_refine", False)
    )

    # candidate difference_type 분포
    cand_type_counter: Counter = Counter()
    for row in results:
        for cand in row["wordcand_self_refine"].get("candidate_rationales", []):
            cand_type_counter[cand.get("difference_type", "unknown")] += 1

    # verified rationale_type 분포
    type_counter: Counter = Counter()
    for row in results:
        for rat in row["wordcand_self_refine"].get("verified_rationales", []):
            type_counter[rat.get("rationale_type", "unknown")] += 1
    ...
    for row in results:
        gold = row["gold_label"]
        wordcand = row["wordcand_self_refine"]
        refined = wordcand["refined_label"]
    ...
    print("  Extraction method          : VitaminC-style cue detector")
    print(f"  Total candidates tested    : {total_cands}")
    print(f"  Total verified rationales  : {total_verified}")
    print(f"  Avg candidates / sample    : {total_cands / len(results):.2f}")
    ...
    print(f"  Candidate type dist        : {dict(cand_type_counter)}")
    print(f"  Rationale type dist        : {dict(type_counter)}")
    print(
        f"  Accuracy (flip-based)      : "
        f"{flip_correct}/{flip_total} = {flip_correct/flip_total:.3f}"
        if flip_total else
        "  Accuracy (flip-based): N/A"
    )
    print(
        f"  Accuracy (conf-drop-based) : "
        f"{conf_correct}/{conf_total} = {conf_correct/conf_total:.3f}"
        if conf_total else
        "  Accuracy (conf-drop-based): N/A"
    )

    # ── 저장 ─────────────────────────────────────────────────
    model_name = get_model_short_name()
    out_path = Path(args.output) if args.output else Path("results") / dataset_name / f"{model_name}_{dataset_name}_wordcand.jsonl"
